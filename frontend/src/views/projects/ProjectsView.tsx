...
export const ProjectsView = () => {
  ...
  const { projects, selectedProject, menuAnchor, modalToDisplay, message } =
    context;
  ...
  useEffect(() => {
    if (!loading) {
      ...
      if (data) {
        const fetchProjectsEvent: FetchedProjectsEvent = {
          type: "HANDLE_FETCHED_PROJECTS",
          data,
        };
  ...
  const onRename = () =>
    dispatch({
      type: "OPEN_MODAL",
      modalToDisplay: "Rename",
    } as OpenModalEvent);
  const onDelete = () =>
    dispatch({
      type: "OPEN_MODAL",
      modalToDisplay: "Delete",
    } as OpenModalEvent);
  const onCloseModal = () =>
