...
const isErrorPayload = (
  payload: GQLCreateProjectPayload
...
export const NewProjectView = () => {
  ...
  const [createProject, { loading, data, error }] =
    useMutation<GQLCreateProjectMutationData>(createProjectMutation);

  const onNameChange = (event) => {
    const value = event.target.value;
    const changeNameEvent: ChangeNameEvent = {
      type: "CHANGE_NAME",
      name: value,
    };
  ...
  const onCreateNewProject = (event) => {
    ...
    const submitEvent: RequestProjectCreationEvent = {
  ...
  useEffect(() => {
    if (!loading) {
      ...
      if (data) {
        const handleResponseEvent: HandleResponseEvent = {
