...
const isChangeNameEvent = (event: NewProjectEvent): event is ChangeNameEvent =>
  !!(event as ChangeNameEvent).name;
const isNameInvalid = (name: string) =>
  name.trim().length < 3 || name.trim().length > 20;
const isCreateProjectSuccessPayload = (
  payload: GQLCreateProjectPayload
): payload is GQLCreateProjectSuccessPayload =>
  payload.__typename === "CreateProjectSuccessPayload";
export const newProjectViewMachine = Machine<
