...
\emph{Visual Studio} w formacie \gls{VSIX}. Po jego zainstalowaniu
...
na rysunku~\ref{rys:prawidlowy-model-uml}. Ponadto, tak przygotowany edytor
może również generować klasy języka C\# pozwalające na reprezentację modelu, a
...
\gls{HTML} zawierającą wszystkie elementy konkretnego typu w modelu.
...
metamodelu jest \emph{Sirius Desktop}~\cite{sirius-desktop-homepage} rozwijany
przez firmę \emph{Eclipse}. Jest to rozszerzenie do~zintegrowanego środowiska
programistycznego (\gls{IDE}) \emph{Eclipse}. Pozwala ono na zaprojektowanie
metamodelu dla wybranej dziedziny używając języka \acrfull{EMF}. Ma on
strukturę drzewiastą i jest
zapisany w pliku \gls{XML} w formacie \emph{ECore} (rozszerzenie
\texttt{*.ecore}). Przykładowy metamodel zdefiniowany w \emph{Sirius Desktop}
...
kodzie języka Java, ich serializację i deserializację. Każdy obiekt metamodelu
...
dostępna w~metamodelu, lub poprzez napisanie wyrażenia w języku \gls{AQL}.
...
klasy zawierają opisu właściwości obiektów modelu.

Czwartym pakietem jest pakiet \texttt{.editor}. Zawiera on definicję wtyczki do
środowiska \emph{Eclipse}, która dostarcza możliwości tworzenia i edycji
...
\emph{JUnit}. Testy te przydają się~gdy~domyślne zachowanie modelu zostanie
...
środowiska \emph{Eclipse}, które jest aplikacją systemową wymagającą instalacji
w systemie użytkownika zanim będzie on~mógł przeglądać i edytować modele.
Od 2018 roku~\cite{sirius-components-repo-first-commit} firma \emph{Eclipse}
pracuje nad~rozwiązaniem \emph{Sirius Web}~\cite{sirius-web-github}, którego
...
\emph{Sirius Web} składa się z 3 komponentów:

\begin{itemize}
	\item serwera aplikacyjnego stworzonego w technologii \emph{Java
		      Spring}~\cite{java-spring-homepage} dostarczającego interfejsu sieciowego \gls{API} w formacie \emph{GraphQL} pozwalającego na wyświetlenie oraz modyfikację modelu,
	\item bazy danych \emph{PostgreSQL}~\cite{postgresql-homepage} przechowującej informacje o modelach i użytkownikach,
	\item aplikacji przeglądarkowej napisanej z wykorzystaniem biblioteki \emph{React}~\cite{react-homepage} umożliwiającej wyświetlenie i modyfikację modeli w formie drzewa obiektów lub diagramu.
\end{itemize}

Metamodele, na których bazuje \emph{Sirius Web} są w tym samym formacie EMF co
metamodele w \emph{Sirius Desktop}. Można zatem w łatwy sposób stworzyć
graficzny edytor modeli zarówno w~formie aplikacji natywnej, jak i
przeglądarkowej. Posiadając istniejący metamodel \gls{EMF} z~\emph{Sirius
	Desktop} można go w krótkim czasie przenieść do \emph{Sirius Web}. Jest
to
...
Zmianie uległ natomiast sposób przechowywania modeli. W \emph{Sirius Desktop}
były one~zapisane w plikach na dysku użytkownika. W \emph{Sirius Web} są one
przechowywane w bazie danych \emph{PostgreSQL}. Taki format zapisu pozwala na
...
serwerem opartych na protokole \emph{WebSocket}~\cite{wikipedia-websocket}).
...
Poprzez zgrupowanie modeli w~projekty wewnątrz aplikacji \emph{Sirius Web}
...
ponieważ wystarczy wysłać adres \gls{URL} innemu użytkownikowi zamiast wysyłać
plik z modelem.
...
użytkownicy po~otworzeniu edytora \emph{Sirius Web} mieli jego najnowszą
wersję.
