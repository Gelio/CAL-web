  ...
  createUnitCall(input: {
      ...
      unitRelease: {
        ...
        pins: [
          ...
          { /* definicja drugiego pinu, podobna do tej powyżej */ }
        ]
      }
  }) {
...
\begin{figure}[!ht]
  ...
  % NOTE: makes the image take up the whole page
  \includegraphics[height=0.93\textheight]{./images/sirius-web-create-unit-call-mutation-handler.pdf}
...
% Źródło:
% http://www.plantuml.com/plantuml/uml/dSx1IWD1383X-pn5w4aBBPvbMLfs4OH5HC5xc8qIo4o6IQOYuhjBRTNR8jw6oV-HxConUaiIE3lLPum4ju-KC3l7OJyzeV2CpfeVcX1qtK_Mz_0U00wdW2u6JQNvLxelHJYU50cX4IITIIwlOMBpRcHyoMhEiGyV8KH1CnZHyKRqTUHAyKY1Kn5AbDqMJZhhE_tsmriKid_YIRNyrPxnx-z85YkNu_E-we5dgc5n2flD6bPNs-rwjRHeHtbkIJu1
% Przekształcono do PDF za pomocą drukowania SVG do PDF w Google Chrome.
% Potem użyto pdfcrop do przycięcia PDF.
%
% @startuml
% interface ISemanticCALValidationRule <<interface>> {
%   validate(ComputationApplicationRelease model): List<Diagnostic>
% }
%
% class DataFlowDirection implements ISemanticCALValidationRule
% class NoDataFlowCycles implements ISemanticCALValidationRule
% class NoDataFlowLoops implements ISemanticCALValidationRule
%
...
\begin{figure}[!hb]
  ...
  \includegraphics[width=0.9\linewidth]{./images/sirius-web-semantic-validation-class-diagram.pdf}
...
\begin{figure}[!t]
  ...
  \includegraphics[width=0.99\linewidth]{./images/sirius-web-library-usage-sequence-diagram.pdf}
...
Diagram sekwencji przedstawiający wykorzystanie udostępnionego modułu
jest widoczny na~rysunku~\ref{rys:sirius-web-library-usage-sequence-diagram}.
Wyświetlenie komponentu \texttt{Workbench} (wywołanie funkcji
\texttt{renderApp}) wymaga przekazania mu parametrów
...
edytora. Po otrzymaniu tych szczegółów można wyświetlić edytor. Po zakończeniu
edycji modelu należy wywołać funkcję \texttt{unmountApp} aby zwolnić zasoby
przeglądarki i ukryć edytor modeli.
